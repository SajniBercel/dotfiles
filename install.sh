...
read -p "Do u want to install basic programs (required most of them required for the configs)? (Y/n): " confirm
if [[ "$confirm" =~ ^([Yy][Ee][Ss]|[Yy])$ || -z "$confirm" ]]; then
...
if [[ "$confirm" =~ ^([Yy][Ee][Ss]|[Yy])$ || -z "$confirm" ]]; then
    read -p "Do u want to remove the current configs (gnu stow will fail if the file already exists)? (Y/n): " confirm
    if [[ "$confirm" =~ ^([Yy][Ee][Ss]|[Yy])$ || -z "$confirm" ]]; then
        sudo rm -rf ~/.bashrc \
            ~/.config/i3/* \
            ~/.config/i3status/*
